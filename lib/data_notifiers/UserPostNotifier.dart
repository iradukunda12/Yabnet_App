...
class UserPostNotifier implements PostNotifier {
  static final UserPostNotifier instance = UserPostNotifier.internal();

  factory UserPostNotifier() => instance;
  WidgetStateNotifier<List<HomePagePostData>> state = WidgetStateNotifier();

  List<HomePagePostData> _data = [];

  UserPostNotifier.internal();
  ...
  Future<bool> saveLastPostTimeChecked(String time) {
    return CacheOperation().saveCacheData(dbReference(Members.database),
        dbReference(Members.last_post_time_checked), time);
  }

  Future<String?> getLastPostTimeChecked() async {
    return await CacheOperation().getCacheData(dbReference(Members.database),
        dbReference(Members.last_post_time_checked));
  }
  ...
  Future<void> preStart() async {
    if (!started) {
      ...
      if (getLatestData().isEmpty) {
        userPostFromTime = await getLastPostTimeChecked() ?? userPostFromTime;
        _fetchPostOnline();
      }
    }
  }
  ...
  Future<void> getLatestPostReceived(String? time) async {
    if (time != null) {
      ...
      await saveLastPostTimeChecked(time);
    }
  }
  ...
  void _fetchPostOnline() async {
    ...
    _getPostLinkedData(
        (await PostOperation().getMembersPostsTime(
            thisUser, userPostFromTime, _fromStart,
            limitBy: 10)),
        false);
  }
  ...
  Future<void> _fetchPostLocal() async {
    final allPost = await PostOperation().getUserLocalData();
    if (allPost.isNotEmpty) {
      ...
      await Future.wait(allPost
          .map((element) async => await createAPostProfileNotifier(
              element.postBy, NotifierType.normal))
          .toList());
      _configure(allPost.toList(), false, false);
    }
  }
  ...
  Future<void> _configure(
    ...
    if (allPost.isNotEmpty && online) {
      await PostOperation()
          .saveUserLocalData(getLatestData())
          .then((value) => value);
    }

    if (allPost.singleOrNull != null && userAdded) {
      await PostNotifier().addUserNewPost(allPost.single);
    }
    sendNewUpdateToUi();
  }

  void addUserPost(Map homePagePostMapData) {
    List<String> postIds = getLatestData().map((e) => e.postId).toList();

    if (!postIds.contains(homePagePostMapData[dbReference(Post.id)])) {
      _getPostLinkedData([homePagePostMapData.cast()], false, userAdded: true);
    }
  }
